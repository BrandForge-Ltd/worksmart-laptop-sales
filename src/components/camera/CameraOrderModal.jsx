...
export default function CameraOrderModal({ isOpen, onClose }) {
  ...
  const handleWhatsAppOrder = (e) => {
    e.preventDefault();
    const message = `Hello, I want to order the V380 PRO 4G LTE Solar Camera.%0A%0A*Name:* ${formData.name}%0A*Phone:* ${formData.phone}%0A*Address:* ${formData.address}%0A%0A*Price:* \u20a6105,000`;
    window.open(`https://wa.me/2348000000000?text=${message}`, '_blank');
  };

  const handlePayNow = (e) => {
    e.preventDefault();
    window.location.href = 'https://paystack.com/pay/sopproducts-camera';
  };
            ...
            <div className="flex min-h-full items-center justify-center p-4" onClick={onClose}>
              ...
              >
                ...
                <div className="mb-8">
                  ...
                  <p className="text-brand-muted text-sm">Please fill in your details to complete your order for the V380 PRO Solar Camera.</p>
                </div>
                ...
                <form className="flex flex-col gap-5">
                  ...
                  <div className="mt-2 text-center">
                    ...
                    <div className="flex items-center justify-center gap-3">
                      {['PAYSTACK', 'FLUTTERWAVE', 'VISA', 'MASTERCARD'].map((b) => (
                        <div key={b} className="bg-gray-100 px-2 py-1 rounded text-[10px] font-bold text-gray-600 border border-gray-200">{b}</div>
                      ))}
                    </div>
                  </div>
                </form>
